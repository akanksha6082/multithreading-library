...
void * exit_thread(void * args) {
    int a = 700;
    void * ptr = &a;
    printf("Thread return using athread_exit\n\n");
...
int main(int argc, char ** argv){
    ...
    fprintf(stdout, "\033[1;34mcase 4 : creating a detached thread \033[0m\n");
    {
        athread_t tid;
        athread_attr_t attr;
        int state;
    ...
    fprintf(stdout, "\n\033[1;34mcase 1 :  Testing SIGUSR1 signal \033[0m\n");
    {
        ...
        fprintf(stdout, "sending SIGUSR1 signal\n");
        
        CHECK(athread_kill(tid, SIGUSR1));
        CHECK(athread_join(tid, &ret));
